...
$columns = array('procedure_id', 'procedure_name','procedure_info');
$column = isset($_GET['column']) && in_array($_GET['column'], $columns) ? $_GET['column'] : $columns[0];
$sort_order = isset($_GET['order']) && strtolower($_GET['order']) == 'desc' ? 'DESC' : 'ASC';
//$perpage = 20;

if (!isset($_SESSION['procedure-list_searchtext'])) {
    $_SESSION['procedure-list_searchtext'] = '';
}
if (isset($_POST['search'])) {
    $_SESSION['procedure-list_searchtext'] = $_POST['searchtext'];
}
$sql = "select * from procedur where procedure_name LIKE '%" . $_SESSION['procedure-list_searchtext'] . "%' ORDER BY " . $column . ' ' . $sort_order;
...
for ($i = 0; $i < @count(@$data); $i++) {
    ...
    if (isset($_REQUEST["{$tem}"])) {
        $_REQUEST["{$tem}"] = 0;
        $sql = "DELETE FROM `procedur` WHERE procedure_id='" . $data[$i]['procedure_id'] . "'";
...
for ($i = 0; $i < @count(@$data); $i++) {
    ...
    if (isset($_REQUEST["{$tem}"])) {
        $_REQUEST["{$tem}"] = 0;
        $_SESSION['editsku'] = $data[$i]['procedure_id'];
        header('location:procedure-edit.php');
        break;
    }
}
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="product-status-wrap">
                    ...
                    <div class="add-product" >                                    
                        ...
                        <a  href="procedure-edit.php">Add Procedure</a>
                    </div>
                    <div>
                            ...
                            <div class="header-top-menu tabl-d-n">
                                <div class="breadcome-heading">
                                    <form method="post" role="search" class="">
                                        ...
                                        <div style="width:200px;float:left;"><input name="searchtext" type="text" placeholder="Search Procedure name....." value="<?php
                                            if (isset($_SESSION['procedure-list_searchtext'])) {
                                                print $_SESSION['procedure-list_searchtext'];
                                            }
                                            ?>" ></div>
                    ...
                    <form action="" method="post" name="form">
                        ...
                        <table >
                            ...
                            <tr>
                                <th><a style="color: #fff" href="procedure-list.php?column=procedure_id&order=<?php echo $asc_or_desc; ?>">Procedure ID <i class=" fa fa-sort<?php echo $column == 'procedure_id' ? '-' . $up_or_down : ''; ?>"></i></a></th>
                                <th><a style="color: #fff" href="procedure-list.php?column=procedure_info&order=<?php echo $asc_or_desc; ?>">Procedure Name <i class=" fa fa-sort<?php echo $column == 'procedure_info' ? '-' . $up_or_down : ''; ?>"></i></a></th>
                                <th><a style="color: #fff" href="procedure-list.php?column=procedure_note&order=<?php echo $asc_or_desc; ?>">Procedure Info<i class=" fa fa-sort<?php echo $column == 'procedure_note' ? '-' . $up_or_down : ''; ?>"></i></a></th>
...
//      else {', '', '', '', '', ''
                            for ($index = 0; $index < @count($data); $index++) {
                                print '<tr>';
                                print "<td>{$data[$index]['procedure_id']}</td>";
                                print "<td>{$data[$index]['procedure_name']}</td>";
                                print "<td>{$data[$index]['procedure_info']}</td>";
                                $edit = "edit" . $index;
                                $trash = "trash" . $index;
                                ?>
