...
if (isset($_POST["delete"])) {
    if ($patient_id != $nextid) {
        ...
        print '<script>alert("Delete Unsuccessful!Please check the patient ID")</script>';
    }
}
...
if (isset($_POST["delete"])) {
    if ($patient_id != $nextid) {
        $sql = "DELETE from patient  WHERE patient='" . $patient_id . "'";
        $result = mysqli_query($conn, $sql);
        print '<script>alert("Delete Successful!")</script>';
        header('Location:' . $_SERVER["PHP_SELF"]);
    } else {
...
if (isset($_SESSION['editsku'])) {
    ...
    $_SESSION['updatepatiendid'] = $patient_id;
    $sql = "SELECT `birthday`, `gender`,  `name`, `address`,`address2`, `city`, `state`, `zipcode`, `phone`, insurance_info  FROM `patient` WHERE patient_id ='" . $patient_id . "'";
    $result = mysqli_query($conn, $sql);
    ...
    if (isset($_REQUEST['search'])) {
        $sql = "SELECT `birthday`, `gender`,  `name`, `address`,`address2`, `city`, `state`, `zipcode`, `phone`, insurance_info  FROM `patient` WHERE patient_id ='" . $_POST['searcheditorder'] . "'";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
        if ($row > 0) {
            ...
            $zipcode = 0;
            $insurance_info = 0;
            print '<script>alert("The patient doesn not exist!")</script>';
        }
    } else {
        $patient_id = $nextid;
        $birthday = 0;
