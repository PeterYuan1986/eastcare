...
$columns = array('doctor_id', 'name', 'profession');
$column = isset($_GET['column']) && in_array($_GET['column'], $columns) ? $_GET['column'] : $columns[0];
$sort_order = isset($_GET['order']) && strtolower($_GET['order']) == 'desc' ? 'DESC' : 'ASC';
//$perpage = 20;

if (!isset($_SESSION['doctor-list_searchtext'])) {
    $_SESSION['doctor-list_searchtext'] = '';
}
if (isset($_POST['search'])) {
    $_SESSION['doctor-list_searchtext'] = $_POST['searchtext'];
}
$sql = "select * from doctor where name LIKE '%" . $_SESSION['doctor-list_searchtext'] . "%' ORDER BY " . $column . ' ' . $sort_order;
...
for ($i = 0; $i < @count(@$data); $i++) {
    ...
    if (isset($_REQUEST["{$tem}"])) {
        $_REQUEST["{$tem}"] = 0;
        $sql = "DELETE FROM `doctor` WHERE doctor_id='" . $data[$i]['doctor_id'] . "'";
...
for ($i = 0; $i < @count(@$data); $i++) {
    ...
    if (isset($_REQUEST["{$tem}"])) {
        $_REQUEST["{$tem}"] = 0;
        $_SESSION['editsku'] = $data[$i]['doctor_id'];
        header('location:doctor-edit.php');
        break;
    }
}
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="product-status-wrap">
                    ...
                    <div class="add-product" >                                    
                        ...
                        <a  href="doctor-edit.php">Add Doctor</a>
                    </div>
                    <div>
                            ...
                            <div class="header-top-menu tabl-d-n">
                                <div class="breadcome-heading">
                                    <form method="post" role="search" class="">
                                        ...
                                        <div style="width:200px;float:left;"><input name="searchtext" type="text" placeholder="Search Dr. Name....." value="<?php
                                            if (isset($_SESSION['doctor-list_searchtext'])) {
                                                print $_SESSION['doctor-list_searchtext'];
                                            }
                                            ?>" ></div>
                    ...
                    <form action="" method="post" name="form">
                        ...
                        <table >
                            ...
                            <tr>
                                <th><a style="color: #fff" href="doctor-list.php?column=doctor_id&order=<?php echo $asc_or_desc; ?>">Doctor ID <i class=" fa fa-sort<?php echo $column == 'doctor_id' ? '-' . $up_or_down : ''; ?>"></i></a></th>
                                <th><a style="color: #fff" href="doctor-list.php?column=name&order=<?php echo $asc_or_desc; ?>">Name <i class=" fa fa-sort<?php echo $column == 'name' ? '-' . $up_or_down : ''; ?>"></i></a></th>
                                <th><a style="color: #fff" href="doctor-list.php?column=profession&order=<?php echo $asc_or_desc; ?>">Profession <i class="fa fa-sort<?php echo $column == 'profession' ? '-' . $up_or_down : ''; ?>"></i></a></th>
                                <th>Setting</th>
...
//      else {', '', '', '', '', ''
                            for ($index = 0; $index < @count($data); $index++) {
                                print '<tr>';
                                print "<td>{$data[$index]['doctor_id']}</td>";
                                print "<td>{$data[$index]['name']}</td>";
                                $sql = "select DISTINCT disease.disease_info from disease NATURAL JOIN doctor WHERE disease_id='" . $data[$index]['profession'] . "'";
                                $result = mysqli_query($conn, $sql);
