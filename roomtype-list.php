...
$columns = array('type_id', 'type_info','type_price');
$column = isset($_GET['column']) && in_array($_GET['column'], $columns) ? $_GET['column'] : $columns[0];
$sort_order = isset($_GET['order']) && strtolower($_GET['order']) == 'desc' ? 'DESC' : 'ASC';
//$perpage = 20;

if (!isset($_SESSION['typed-list_searchtext'])) {
    $_SESSION['typed-list_searchtext'] = '';
}
if (isset($_POST['search'])) {
    $_SESSION['typed-list_searchtext'] = $_POST['searchtext'];
}
$sql = "select * from typed where type_id LIKE '%" . $_SESSION['typed-list_searchtext'] . "%' ORDER BY " . $column . ' ' . $sort_order;
...
for ($i = 0; $i < @count(@$data); $i++) {
    ...
    if (isset($_REQUEST["{$tem}"])) {
        $_REQUEST["{$tem}"] = 0;
        $sql = "DELETE FROM `typed` WHERE type_id='" . $data[$i]['type_id'] . "'";
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="product-status-wrap">
                    ...
                    <div class="add-product" >                                    
                        ...
                        <a  href="roomtype-edit.php">Add Room Type</a>
                    </div>
                    <div>
                            ...
                            <div class="header-top-menu tabl-d-n">
                                <div class="breadcome-heading">
                                    <form method="post" role="search" class="">
                                        ...
                                        <div style="width:200px;float:left;"><input name="searchtext" type="text" placeholder="Search Room Type ID....." value="<?php
                                            if (isset($_SESSION['typed-list_searchtext'])) {
                                                print $_SESSION['typed-list_searchtext'];
                                            }
                                            ?>" ></div>
                    ...
                    <form action="" method="post" name="form">
                        ...
                        <table >
                            ...
                            <tr>
                                <th><a style="color: #fff" href="roomtype-list.php?column=type_id&order=<?php echo $asc_or_desc; ?>">Room Type ID <i class=" fa fa-sort<?php echo $column == 'type_id' ? '-' . $up_or_down : ''; ?>"></i></a></th>
                                <th><a style="color: #fff" href="roomtype-list.php?column=type_info&order=<?php echo $asc_or_desc; ?>">Type Info <i class=" fa fa-sort<?php echo $column == 'type_info' ? '-' . $up_or_down : ''; ?>"></i></a></th>
                                <th><a style="color: #fff" href="roomtype-list.php?column=type_price&order=<?php echo $asc_or_desc; ?>">Type Price <i class=" fa fa-sort<?php echo $column == 'type_price' ? '-' . $up_or_down : ''; ?>"></i></a></th>
...
//      else {', '', '', '', '', ''
                            for ($index = 0; $index < @count($data); $index++) {
                                print '<tr>';
                                print "<td>{$data[$index]['type_id']}</td>";
                                print "<td>{$data[$index]['type_info']}</td>";
